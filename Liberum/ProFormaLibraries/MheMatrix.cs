...
namespace ProFormaLibraries
{
    ...
    public class MheMatrix
    {
        ...
        public static string MheMatrixHtml()
        {
            string MatrixHtmlContent = "";  // Blank out
            int TrainingExpiry = 1825;      // Training is to expire after 5 years (5 * 365 = 1825) -  we'll paint them red after expiration
            int TrainingWarning = 365;      // When it would get to 365 days prior to training expiration, we'll paint them orange


            MatrixHtmlContent = MatrixHtmlContent +
                @"<!DOCTYPE html><html><head><title>Antolin Redditch MHE Matrix</title><style>
            ...
            MatrixHtmlContent = MatrixHtmlContent + @""" alt = ""Antolin"" style = ""padding: 5pt; max-width: 50%; height: auto;""></div>



                    <div style = ""background-color:white; float:inline-end; text-align: right; padding-left: 10px;""><p><h3>MHE MATRIX";

          

            MatrixHtmlContent = MatrixHtmlContent + "&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</h3></p></div></div><hr>";
            ...
            MatrixHtmlContent = MatrixHtmlContent + $@"
                 ...
                 <tr>
                        ...
                        <i>Redditch, {DateTime.Now.ToString("dd/MM/yyyy")}</i><br><br><hr>
                        <div>Powered by Pawel Liberski. All rights reserved</div>
                        </div></body></html>";

            return MatrixHtmlContent;
        }

        public static void CreateHtmlMatrix()
        {
            string HtmlOutput = MheMatrixHtml();
